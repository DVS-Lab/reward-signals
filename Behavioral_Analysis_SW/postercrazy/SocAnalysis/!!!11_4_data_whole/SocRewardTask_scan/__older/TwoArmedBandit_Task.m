function TwoArmedBandit_Task(subjectnum,cbtype,whichhuman)
...
try
    ...
    while keep_going
        ...
        longest_msg = 'Index Finger = Left Button.      Middle Finger = Right Button.  '; %You''ll see the outcome of one decision at the end of the task.
        [normBoundsRect, ~] = Screen('TextBounds', mywindow, longest_msg);
        Screen('TextStyle', mywindow, 1);
        Screen('DrawText', mywindow, longest_msg, (centerhoriz-(normBoundsRect(3)/2)), centervert-220, black);
        Screen('TextStyle', mywindow, 0);
        Screen('FillRect', mywindow, white, LeftRect);
        Screen('FillRect', mywindow, white, RightRect);
        Screen('FrameRect',mywindow, black, LeftRect,5);
        Screen('FrameRect',mywindow, black, RightRect,5);
        ...
        while ~press
            ...
            if find(responsecode) == L_arrow %LEFT
                Screen('FrameRect',mywindow,red,LeftRect,5);
                press = 1;
                Screen('Flip', mywindow);
            elseif find(responsecode) == R_arrow %right
                Screen('FrameRect',mywindow,red,RightRect,5);
                press = 1;
                Screen('Flip', mywindow);
    ...
    %timing
    fix1_list = [repmat(2,1,20) repmat(4,1,10) repmat(6,1,6)];
    fix2_list = [repmat(2,1,20) repmat(4,1,10) repmat(6,1,6)];
    fix3_list = [repmat(2,1,20) repmat(4,1,10) repmat(6,1,6)];
    fix4_list = [repmat(2,1,20) repmat(4,1,10) repmat(6,1,6)];
    partner_dec1 = 0.75;
    partner_dec2 = 2.25;
    self_dec = 3;
    infodur = 1.5;
    ...
    %point distributions | be careful to index appropriately
    % set blocks of 24 trials to be spread across all 144 trials
    highval = [ones(1,1) ones(1,3).*2 ones(1,8).*3  ones(1,12).*4];
    lowval = [ones(1,12) ones(1,8).*2 ones(1,3).*3  ones(1,1).*4];
    
    deck1 = [Shuffle(highval) Shuffle(lowval), ...
        Shuffle(highval) Shuffle(lowval), ...
        Shuffle(highval) Shuffle(lowval)];
    
    deck2 = [Shuffle(lowval) Shuffle(highval), ...
        Shuffle(lowval) Shuffle(highval), ...
        Shuffle(lowval) Shuffle(highval)];
    ...
    for gametype = partnerorder
        ...
        if gametype == 1
            type = 'Human';
            if strcmp(whichhuman,'Steph')
                partner_texture = steph_texture;
            else
                partner_texture = dave_texture;
            end
        elseif gametype == 2
            type = 'Computer';
            partner_texture = comp_texture;
        end
        instruction_msg = sprintf('Run %d of %d: You need %d more circles to earn money!', count, length(partnerorder), needed_points-point_count);
        ...
        Screen('DrawText', mywindow, 'Each option yields different amounts of circles.', (centerhoriz-(normBoundsRect(3)/2)), centervert, black);
        Screen('DrawText', mywindow, 'Get as many circles as you can to earn money.', (centerhoriz-(normBoundsRect(3)/2)), (centervert+(35*scale_res(2))), black);
