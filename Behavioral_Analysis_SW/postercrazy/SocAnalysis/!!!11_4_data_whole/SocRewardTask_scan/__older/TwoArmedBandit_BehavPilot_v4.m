function TwoArmedBandit_BehavPilot_v4(subjectnum)
...
try
    ...
    is_catch = [1 1 1 1 2 2 2 2 zeros(1,36)];
    ...
    for b = 1:length(randblocks)
        ...
        msg1 = sprintf('Run %d of %d: need %d more points.', b, length(randblocks),points_needed-point_total);
        ...
        for k = 1:ntrials
            ...
            elseif is_catch(k) == 1 %inf only with rating
                ...
                %%--DRAW SLIDER
                Screen('TextSize', mywindow, floor((30*scale_res(2))));
                bid_msg = sprintf('Indicate to what extent you feel this way right now: %s',words{1});
