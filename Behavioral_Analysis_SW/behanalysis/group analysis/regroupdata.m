...
% addpath(fullfile(maindir,'rw'));
    ...
    for s = 1:length(subjects)
        ...
        for r = 1:length(blocks)
            ...
            ratingdata = [data.soc_win;data.is_catch;data.rating;data.partner]';
            worddata = {data.word}';
            soc_win = [soc_win; ratingdata(:,1)];
            is_catch = [is_catch;ratingdata(:,2)];
            word = [word; worddata];
            rating = [rating; ratingdata(:,3)];
            partner = [partner;ratingdata(:,end)];
        end
        ...
        %%%%%practice
            ...
            pracrating = ratingdata(:,3);
            pracpartner = ratingdata(:,end);
