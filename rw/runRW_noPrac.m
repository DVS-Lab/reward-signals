function [loglike_e, alpha_e, beta_e, pe_e]  = runRW_noPrac(decision, outcome, alpha, beta)
...
try
    ...
    %Set initial values
    pvt = 8;
    vD1(1:pvt) = mean(data(data(1:pvt,1)==1,2));
    vD2(1:pvt) = mean(data(data(1:pvt,1)==2,2));
    pD1(1:pvt) = 1/2;
    pD2(1:pvt) = 1/2;
    loglike_e = 0;
    pe(1:pvt) = 0;
    ...
    for trialnum = pvt:size(data,1)
        ...
        if ~data(trialnum,1) || data(trialnum,1) == 11 || data(trialnum,1) == 22
            pe(trialnum) = 0; %remove later
